...
gtf=$(dirname $reference)/$(basename $reference .fasta).gtf
...
tag_cells="${dropseq_root}/TagBamWithReadSequenceExtended SUMMARY=${outdir}/unaligned_tagged_Cellular.bam_summary.txt \
    BASE_RANGE=28-33:49-54:70-75 BASE_QUALITY=10 BARCODED_READ=1 DISCARD_READ=false TAG_NAME=XC NUM_BASES_BELOW_QUALITY=1 INPUT=${unmapped_bam}"

tag_molecules="${dropseq_root}/TagBamWithReadSequenceExtended SUMMARY=${outdir}/unaligned_tagged_Molecular.bam_summary.txt \
BASE_RANGE=76-81 BASE_QUALITY=10 BARCODED_READ=1 DISCARD_READ=true TAG_NAME=XM NUM_BASES_BELOW_QUALITY=1"

filter_bam="${dropseq_root}/FilterBAM TAG_REJECT=XQ"


trim_poly_a="${dropseq_root}/PolyATrimmer OUTPUT=${tagged_unmapped_bam} OUTPUT_SUMMARY=${outdir}/polyA_trimming_report.txt MISMATCHES=0 NUM_BASES=10"

# Stage 2: alignment
sam_to_fastq="java -Xmx100g -jar ${picard_jar} SamToFastq INPUT=${tmpdir}/unaligned_mc_tagged_polyA_filtered.bam"
star_align="$star_executable --genomeDir ${genomedir} --runThreadN 22 --outFileNamePrefix ${tmpdir}/star."

# Stage 3: sort aligned reads (STAR does not necessarily emit reads in the same order as the input)
sort_aligned="java -Dsamjdk.buffer_size=131072 -XX:GCTimeLimit=50 -XX:GCHeapFreeLimit=10 -Xmx40000m -jar ${picard_jar} \
...
merge_bam="java -Xmx100g -jar ${picard_jar} MergeBamAlignment REFERENCE_SEQUENCE=${reference} UNMAPPED_BAM=${tagged_unmapped_bam} \
ALIGNED_BAM=${aligned_sorted_bam} INCLUDE_SECONDARY_ALIGNMENTS=false PAIRED_RUN=false"
tag_with_gene_exon="${dropseq_root}/TagReadWithGeneExon O=${outdir}/star_gene_exon_tagged.bam ANNOTATIONS_FILE=${gtf} TAG=GE CREATE_INDEX=true"

# Stage 5: DigitalExpression
Digital_Expression="${dropseq_root}/DigitalExpression OUTPUT=${outdir}/_dge.txt.gz SUMMARY=${outdir}/_dge.summary.txt NUM_CORE_BARCODES=10000 CELL_BARCODE_TAG=XC MOLECULAR_BARCODE_TAG=XM GENE_EXON_TAG=GE STRAND_TAG=GS EDIT_DISTANCE=2 READ_MQ=10 MIN_BC_READ_THRESHOLD=0  RARE_UMI_FILTER_THRESHOLD=0.0  CREATE_INDEX=false"
...
then
     ...
     $tag_cells OUTPUT=/dev/stdout COMPRESSION_LEVEL=0 | \
       ...
       $trim_poly_a INPUT=/dev/stdin OUTPUT=/dev/stdout COMPRESSION_LEVEL=0 | \
     ...
     $echo_prefix $trim_poly_a INPUT=$tmpdir/unaligned_tagged_filtered.bam
     ...
     # Stage 5
     $echo_prefix $Digital_Expression INPUT=$tmpdir/star_gene_exon_tagged.bam
